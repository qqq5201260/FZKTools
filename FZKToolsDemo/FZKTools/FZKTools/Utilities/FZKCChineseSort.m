...
@implementation FZKCChineseSort
...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr{
    ...
    {
        NSString *pinyin = [((FZKCChineseSort*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((FZKCChineseSort*)object).pinYin substringToIndex:1];
        NSString *string = ((FZKCChineseSort*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr{
    ...
    {
        FZKCChineseSort *chineseString = [[FZKCChineseSort alloc]init];
        ...
        chineseString.string = [FZKCChineseSort RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)sortObjectArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    NSString *tempString;
    //拼音分组
    for (FZKCChineseSort* object in tempArray) {
