...
static inline NSString *cachePath() {
    //    return [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/FZKNetworkingCaches"];
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
    NSString *docPath = [paths lastObject];
    return [docPath stringByAppendingPathComponent:@"FZKNetworkingCaches"];
}
...
+ (void)clearCaches {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:directoryPath isDirectory:nil]) {
        ...
        if (error) {
            NSLog(@"FZKNetworking clear caches error: %@", error);
        } else {
            NSLog(@"FZKNetworking clear caches ok");
        }
    }
}
...
+ (void)cancelAllRequest {
    @synchronized(self) {
        [[self allTasks] enumerateObjectsUsingBlock:^(FZKURLSessionTask * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([task isKindOfClass:[FZKURLSessionTask class]]) {
                [task cancel];
            }
        }];
...
+ (FZKURLSessionTask *)_requestWithUrl:(NSString *)url
                          refreshCache:(BOOL)refreshCache
                             httpMedth:(NSUInteger)httpMethod
                                params:(NSDictionary *)params
                              progress:(FZKDownloadProgress)progress
                               success:(SuccessBlock)success
                                  fail:(FailureBlock)fail {
    //设置请求超时重试次数
...
+ (AFHTTPSessionManager *)manager {
    ...
    switch (sg_requestType) {
        case kFZKRequestTypeJSON: {
            manager.requestSerializer = [AFJSONRequestSerializer serializer];
            break;
        }
        case kFZKRequestTypePlainText: {
    ...
    switch (sg_responseType) {
        case kFZKResponseTypeJSON: {
            manager.responseSerializer = [AFJSONResponseSerializer serializer];
            break;
        }
        case kFZKResponseTypeXML: {
            manager.responseSerializer = [AFXMLParserResponseSerializer serializer];
            break;
        }
        case kFZKResponseTypeData: {
...
+ (NSString *)encodeUrl:(NSString *)url {
    return [self FZK_URLEncode:url];
}

+ (NSString *)FZK_URLEncode:(NSString *)url {
